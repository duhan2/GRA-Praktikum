	...
	void onNewData(const royale::DepthData* data)
	{
		...
		if (case2 == 1) {
			...
			if(zImage2.data != NULL) {
				...
				rectangle(tempzImage, Rect(x_koor, y_koor, width, height), weiss, -1, 8, 0);

				imshow("DIF", tempzImage);

				int schwelle; // Die Schwelle muss herausgefunden werden 

				/*
				for (int x = 0; x < tempzImage.rows; x++) {
					for (int y = 0; y < tempzImage.cols; y++) {
						cout << tempzImage.at<float>(x, y);
					}
				}*/


				//Bevor das alles gemacht wird , muss die Schwelle bekannt sein!

				//Erstelle 2 Binärbilder
				Mat gray_temp,minbinaryPic1,maxbinaryPic2,ergebnis;
				cvtColor(tempzImage, gray_temp, CV_BGR2GRAY);
				threshold(gray_temp, minbinaryPic1, schwelle, 255, THRES_BINARY); //alle < schwelle = schwarz
				threshold(gray_temp, maxbinaryPic2, schwelle, 255, THRES_BINARY_INV); // alle > schwelle = schwarz
				imshow("minbinaryPic1", minbinaryPic1);
				imshow("maxbinaryPic2", maxbinaryPic2);
				
				//Opening auf die Binärbilder
				Mat element = getStructuringElement(MORPH_CROSS, Size(3, 3));
				morphologyEx(minbinaryPic1, MORPH_OPEN, element);
				morphologyEx(maxbinaryPic2, MORPH_OPEN, element);

				//Einfärben der Pixel im Ergebnisbild
				vector<Mat>channels;
				Mat green = Mat::zeros(Size(gray_temp.rows, gray_temp.cols), CV_8UC1); // Ist CV_8UC1 richtig? Auch bei binary Pictures?
				vector.push_back(minbinaryPic1); // Blau
				vector.push_back(green); //Grün
				vector.push_back(maxbinaryPic2); // Rot
